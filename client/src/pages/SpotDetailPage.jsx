...
export default function SpotDetailPage() {
  ...
  if (loading) return <div className="min-h-screen bg-surface flex items-center justify-center"><p className="text-on-surface-variant font-label">Loading...</p></div>
  if (!spot) return <div className="min-h-screen bg-surface flex items-center justify-center"><p className="text-on-surface-variant font-label">Spot not found.</p></div>

  const currentHour = new Date().getHours()
  const chartHour = Math.min(Math.floor((currentHour - 8) / 1.5), (spot.hourlyBusyness?.length ?? 10) - 1)

  return (
    <div className="min-h-screen bg-surface pb-32">
      <AppHeader mode="detail" title={spot.name} />
      ...
      <main className="pt-20 px-4 max-w-2xl mx-auto">
        ...
        <div className="relative rounded-xl overflow-hidden h-52 mb-6 bg-surface-container-high">
          ...
          <div className="absolute bottom-4 left-4 z-20">
            <h2 className="font-headline font-bold text-white text-2xl leading-tight">{spot.name}</h2>
          ...
          <div className="absolute top-3 right-3 z-20 bg-black/40 backdrop-blur-sm px-3 py-1 rounded-full flex items-center gap-1">
            ...
            <span className="text-white/70 text-[10px] font-label">Updated {Math.floor((Date.now() - new Date(spot.lastUpdated)) / 60000)}m ago</span>
          </div>
          {/* Placeholder gradient for hero image */}
          <div className="w-full h-full" style={{ background: 'linear-gradient(135deg, #861f41 0%, #2e3235 100%)' }} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <BentoStat
            icon="power"
            label="Outlets"
            value={spot.outletStatus === 'plenty' ? '12+' : spot.outletStatus === 'available' ? '6+' : '2'}
            sublabel={spot.outletStatus === 'plenty' ? 'High availability' : spot.outletStatus === 'scarce' ? 'Limited' : 'Available'}
          />
          <BentoStat
            icon="chair"
            label="Space Size"
        ...
        {spot.amenities?.length > 0 && (
          <div className="mb-6">
            <p className="text-xs font-label uppercase tracking-widest text-on-surface-variant mb-3">Features & Amenities</p>
            <div className="grid grid-cols-2 gap-3">
              {spot.amenities.map(a => {
                const meta = AMENITY_META[a]
                if (!meta) return null
                return (
                  <div key={a} className="flex items-center gap-3 p-4 bg-surface-container-low rounded-xl">
                    ...
                    <div>
                      <p className="font-label font-semibold text-on-surface text-sm">{meta.label}</p>
                      <p className="text-xs font-label text-on-surface-variant">{meta.desc}</p>
                    </div>
                  </div>
                )
              })}
